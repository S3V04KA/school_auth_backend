...
from app.api.dependencies.auth import validate_is_admin
...
@router.get("/id/{role_id}", dependencies=[Depends(validate_is_admin)], description='Required admin access')
async def role(role_id: int, db_session: DBSessionDep) -> RoleInDB:
    return await get_role(db_session, role_id)


@router.put("/", dependencies=[Depends(validate_is_admin)], description='Required admin access')
...
@router.post("/", dependencies=[Depends(validate_is_admin)])
